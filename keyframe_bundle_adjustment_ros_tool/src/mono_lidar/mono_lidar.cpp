...
{
    // Load parameters and set up diagnostics.
    ...
    // Set up synchronizer for tracklets and camera info.
    ...
    // Get extrinsics from TF: transform from vehicle to camera.
    ...
    try {
        Eigen::Affine3d ext_aff = tf2::transformToEigen(tf_buffer_.lookupTransform(
            interface_.calib_target_frame_id, interface_.calib_source_frame_id,
        ...
        ROS_ERROR_STREAM("MonoLidar: Using identity for calibration");
...
MonoLidar::~MonoLidar() {
    // Dump map (landmarks and poses) on shutdown.
...
void MonoLidar::callbackSubscriber(const TrackletsMsg::ConstPtr& tracklets_msg,
    ...
    if (tracklets_msg->stamps.empty()) {
        ROS_DEBUG_STREAM("MonoLidar: No tracklet data yet; returning");
        return;
    }
    ...
    auto camera = std::make_shared<keyframe_bundle_adjustment::Camera>(
    ...
    if (!bundle_adjuster_->keyframes_.empty()) {
        ...
        if (has_external_prior) {
            ...
            Eigen::Affine3d pose_prior_cur_kf, pose_prior_kf_orig = last_kf.getEigenPose();
            ROS_DEBUG_STREAM("MonoLidar: Getting prior from TF...");
            geometry_msgs::TransformStamped motion_cur_kf;
            try {
                motion_cur_kf = tf_buffer_.lookupTransform(
                    interface_.prior_vehicle_frame, cur_ts_ros,
                    interface_.prior_vehicle_frame, last_ts_ros,
                    interface_.prior_global_frame, ros::Duration(interface_.tf_waiting_time));
            ...
            ss << "Duration to get pose prior from TF = "
               ...
               << " ms" << std::endl;
            ROS_DEBUG_STREAM("MonoLidar: pose prior for keyframe =\n" << pose_prior_cur_kf.matrix());
            ...
            // Compute motion using the 5-point algorithm.
            ...
            if (kf_ptrs.size() > 1) {
                ...
                double norm_trans = std::max(0.0001, motion_vehicle_t1_t0.translation().norm());
                motion_vehicle_t1_t0.translation() = motion_vehicle_t1_t0.translation() / norm_trans *
                                                      speed * (cur_ts_ros.toSec() - keyframe_bundle_adjustment::convert((*r_it)->timestamp_));
            }
            pose_prior_keyframe_origin = motion_vehicle_t1_t0 * kf_ptrs.back()->getEigenPose();
        }

        // Create new keyframe using computed pose.
        ...
        auto cur_frame = std::make_shared<keyframe_bundle_adjustment::Keyframe>(
            cur_ts_ros.toNSec(), tracklets, camera, pose_prior_keyframe_origin,
            keyframe_bundle_adjustment::Keyframe::FixationStatus::None, ground_plane);

        // If the prior was unscaled, adjust the keyframe pose.
        if (!has_external_prior) {
            auto start_adjust = std::chrono::steady_clock::now();
            ...
            std::cout << "---------------- Motion-only Adjustment ----------------" << std::endl;
            std::cout << summary_motion_only << std::endl;
            ss << "Duration for pose-only adjustment = "
               << std::chrono::duration_cast<std::chrono::milliseconds>(
                      std::chrono::steady_clock::now() - start_adjust).count()
               << " ms" << std::endl;
        }

        ROS_DEBUG_STREAM("Motion prior delta =\n" <<
        ...
        int64_t select_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                                        std::chrono::steady_clock::now() - start_select).count();
        ss << "Duration to select keyframes = " << select_duration << " ms\n";

        // Add selected keyframes to the bundle adjustment.
        ...
        ss << "Duration to push keyframes = " 
           << std::chrono::duration_cast<std::chrono::milliseconds>(
                  std::chrono::steady_clock::now() - start_push).count()
           << " ms\n";
        ROS_DEBUG_STREAM("MonoLidar: Total keyframes = " << bundle_adjuster_->keyframes_.size());
        ROS_INFO_STREAM("MonoLidar: Selected keyframes = " << selected_frames.size());
        ROS_INFO_STREAM("MonoLidar: Active keyframes = " << bundle_adjuster_->active_keyframe_ids_.size());
        ROS_INFO_STREAM("MonoLidar: Active landmarks = " << bundle_adjuster_->active_landmark_ids_.size());
        ROS_INFO_STREAM("MonoLidar: Selected landmarks = " << bundle_adjuster_->selected_landmark_ids_.size());

        // Run bundle adjustment if enough keyframes exist and time threshold is met.
        if (bundle_adjuster_->keyframes_.size() > 2 &&
            ...
            bundle_adjuster_->deactivateKeyframes(interface_.min_number_connecting_landmarks,
                                                  3, interface_.max_size_optimization_window);
            ...
            ss << "Duration to solve = " 
               << std::chrono::duration_cast<std::chrono::milliseconds>(
                      std::chrono::steady_clock::now() - start_solve).count()
               << " ms\n";
        ...
        // Dump pose to file if a dump path is specified.
        if (!interface_.dump_path.empty()) {
            ...
            if (bundle_adjuster_->getKeyframe().timestamp_ == tracklets_msg->stamps[0].toNSec()) {
                ...
                // Dump current prior pose.
                pose_origin_camera = trf_camera_vehicle * pose_prior_keyframe_origin.inverse() *
                                      trf_camera_vehicle.inverse();
        ...
        // First keyframe: use the oldest feature timestamp.
    ...
    // Publish paths, landmarks, and planes.
    auto start_publish = std::chrono::steady_clock::now();
    ...
    ss << "Duration to publish = " 
       << std::chrono::duration_cast<std::chrono::milliseconds>(
              std::chrono::steady_clock::now() - start_publish).count()
       << " ms\n";
    ...
    ss << "Duration to send pose TF = " 
       << std::chrono::duration_cast<std::chrono::milliseconds>(
              std::chrono::steady_clock::now() - start_send_tf).count()
       << " ms\n";

    updater_.update();

    auto total_duration = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::steady_clock::now() - start_time);
    ss << "Total callback time = " << total_duration.count() << " sec\n";
...
void MonoLidar::reconfigureRequest(const ReconfigureConfig& config, uint32_t level) {
    ...
    // Create a new landmark selector.
    ...
    // Set up landmark sparsification using voxelgrid.
    ...
    // Set up parameters for adding depth.
    ...
    // Load outlier and shrubbery labels.
    ...
    bundle_adjuster_->labels_["shrubbery"] =
        keyframe_bundle_adjustment_ros_tool::helpers::loadSetFromYaml(interface_.outlier_labels_yaml, "shrubbery_labels");
    ss.str("");
